...
Begin Window WindowFile
   ...
   Begin TextField TextFieldOutputPath
      ...
      Width           =   451
   End
   Begin ShellWaveform GenerateWaveform
      Arguments       =   ""
      Backend         =   ""
      Canonical       =   False
      ExecuteMode     =   1
...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  self.GenerateWaveform.Execute( self.audioFile )
...
#tag Events GenerateWaveform
	#tag Event
		Sub Completed(p as Picture, data as Dictionary)
		  // TODO check properly if we got any metadata back
		  
		  if data.Lookup( "title", "" ) <> "" then
		    
		    self.Title = self.Title + " (" + data.Value( "title" ) + ")"
		    
		  end if
		  
		  self.Title = self.Title + " " + data.Value( "duration (string trimmed)" )
		  
		  // System.DebugLog( data.DebugLog() )
		  
		  // TODO check if p is nil ...
		  
		  self.Waveform.Set( p, data.Value( "duration (double)" ) )
